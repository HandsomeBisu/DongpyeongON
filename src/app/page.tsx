...
import { Activity, Bell, Clock3, Flame, Home, Inbox, LogIn, Megaphone, MessageCircle, Music2, Radio, Search, Utensils } from "lucide-react";
  ...
  return <div className="min-h-screen bg-[#f5f5f7]">
    ...
    <main className="page-enter mx-auto grid max-w-6xl gap-8 px-5 py-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start lg:py-10">
      ...
      <aside className="space-y-6 lg:sticky lg:top-[98px]">
        <SideCard icon={<Utensils size={22}/>} title="오늘의 급식" badge="점심">
          <div className="grid min-h-[150px] place-items-center rounded-2xl bg-[#f5f5f7] p-5 text-center text-sm text-[var(--muted)] shadow-inner">오늘의 급식 정보가<br/>아직 등록되지 않았어요.</div>
        </SideCard>
        <SideCard icon={<Clock3 size={22}/>} title="오늘 시간표">
          <div className="grid min-h-[150px] place-items-center rounded-2xl bg-[#f5f5f7] p-5 text-center"><div><p className="text-sm leading-6 text-[var(--muted)]">우리 반 시간표를 확인하려면<br/>먼저 로그인해 주세요.</p><Link href="/login" className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-[#007aff] px-5 py-2.5 text-sm font-semibold text-white shadow-md shadow-blue-500/20 hover:scale-[1.03]"><LogIn size={15}/>로그인하기</Link></div></div>
        </SideCard>
        <SideCard icon={<Megaphone size={22}/>} title="학교/학생회 공지">
          <div className="grid min-h-[130px] place-items-center rounded-2xl bg-[#f5f5f7] p-5 text-center text-sm text-[var(--muted)]">등록된 공지사항이 없어요.</div>
        </SideCard>
      </aside>
    </main>

    <nav aria-label="모바일 메뉴" className="glass-bar fixed inset-x-0 bottom-0 z-50 grid grid-cols-4 border-t border-b-0 px-2 pb-[max(.35rem,env(safe-area-inset-bottom))] pt-1 md:hidden">{mobileLinks.map(({href,label,icon:Icon}) => <Link key={href} href={href} className={`flex flex-col items-center gap-0.5 rounded-xl py-1.5 text-[10px] font-medium ${href === "/" ? "text-[#007aff]" : "text-[var(--muted)]"}`}><Icon size={20}/>{label}</Link>)}</nav>
  </div>;
}

function DashboardSection({ icon, title, trailing, children }: { icon: React.ReactNode; title: string; trailing?: React.ReactNode; children: React.ReactNode }) {
  return <section><div className="mb-4 flex min-h-10 items-center gap-2"><span className="text-[var(--foreground)]">{icon}</span><h2 className="text-2xl font-bold tracking-[-0.03em]">{title}</h2>{trailing && <div className="ml-auto">{trailing}</div>}</div>{children}</section>;
}

function EmptyPanel({ text, action, href, className = "" }: { text: string; action?: string; href?: string; className?: string }) {
  return <div className={`ios-card grid place-items-center p-8 text-center ${className}`}><div><p className="text-sm text-[var(--muted)]">{text}</p>{action && href && <Link href={href} className="mt-3 inline-block text-sm font-semibold text-[#007aff] hover:opacity-70">{action} →</Link>}</div></div>;
}

function SideCard({ icon, title, badge, children }: { icon: React.ReactNode; title: string; badge?: string; children: React.ReactNode }) {
  return <section className="ios-card stagger-item p-5"><div className="mb-5 flex items-center gap-2"><span>{icon}</span><h2 className="text-xl font-bold tracking-[-0.025em]">{title}</h2>{badge && <span className="ml-auto rounded-lg border border-black/8 bg-white px-3 py-1.5 text-xs font-semibold shadow-sm">{badge}</span>}</div>{children}</section>;
}
