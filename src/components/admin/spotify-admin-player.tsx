...
export function SpotifyAdminPlayer({ queue, onTrackCompleted }: { queue: SongRequestRecord[]; onTrackCompleted: (requestId: string) => Promise<boolean> }) {
  ...
  return <>{spotifySdk}<section className="mb-5 overflow-hidden rounded-3xl bg-linear-to-br from-[#282828] via-[#1b1b1b] to-[#101010] p-5 text-white shadow-xl sm:p-6"><div className="flex flex-col gap-5 lg:flex-row lg:items-center"><div className="flex min-w-0 flex-1 items-center gap-4">{track?.album.images[0]?.url ? <Image src={track.album.images[0].url} alt={`${track.album.name} 앨범 표지`} width={80} height={80} className="size-20 shrink-0 rounded-2xl object-cover shadow-lg"/> : <span className="grid size-20 shrink-0 place-items-center rounded-2xl bg-white/8 text-white/40"><Music2 size={29}/></span>}<div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[.12em] text-[#1ed760]">Web player ready</p><h2 className="mt-1 truncate text-xl font-bold">{track?.name ?? "재생 중인 곡이 없어요"}</h2><p className="mt-1 truncate text-sm text-white/55">{track?.artists.map((artist) => artist.name).join(", ") ?? "승인 목록을 재생해 보세요."}</p></div></div><div className="flex flex-col items-center gap-3"><div className="flex items-center gap-3"><button type="button" aria-label="이전 곡" onClick={() => void changeTrack("previous")} className="grid size-9 place-items-center rounded-full text-white/65 hover:text-white"><SkipBack size={20} fill="currentColor"/></button><button type="button" aria-label={playback?.paused === false ? "일시정지" : "재생"} onClick={() => void playerRef.current?.togglePlay()} className="grid size-12 place-items-center rounded-full bg-white text-black hover:scale-105">{playback?.paused === false ? <Pause size={20} fill="currentColor"/> : <Play size={20} fill="currentColor" className="ml-0.5"/>}</button><button type="button" aria-label="다음 곡" onClick={() => void changeTrack("next")} className="grid size-9 place-items-center rounded-full text-white/65 hover:text-white"><SkipForward size={20} fill="currentColor"/></button></div><div className="flex w-full min-w-64 items-center gap-2 text-[10px] text-white/45"><span>{formatDuration(position)}</span><input aria-label="재생 위치" type="range" min={0} max={Math.max(duration, 1)} value={Math.min(position, duration || 1)} onChange={(event) => void seek(Number(event.target.value))} className="h-1 flex-1 accent-white"/><span>{formatDuration(duration)}</span></div></div><div className="flex flex-wrap items-center gap-2 lg:justify-end"><label className="flex items-center gap-2 rounded-full bg-white/8 px-3 py-2"><Volume2 size={15} className="text-white/55"/><input aria-label="음량" type="range" min={0} max={1} step={0.05} value={volume} onChange={(event) => { const next = Number(event.target.value); setVolume(next); void playerRef.current?.setVolume(next); }} className="w-20 accent-[#1ed760]"/></label><button type="button" disabled={!queue.length || busy} onClick={() => void startQueue()} className="inline-flex h-10 items-center gap-1.5 rounded-full bg-[#1ed760] px-4 text-sm font-bold text-black hover:scale-[1.02] disabled:opacity-40">{busy ? <LoaderCircle size={16} className="animate-spin"/> : <ListMusic size={16}/>}목록 재생</button><button type="button" disabled={busy} onClick={() => void disconnect()} className="grid size-10 place-items-center rounded-full bg-white/8 text-white/55 hover:bg-white/12 hover:text-white" aria-label="Spotify 연결 해제"><RotateCcw size={16}/></button></div></div>{message && <p role="status" className="mt-4 rounded-xl bg-white/6 px-3 py-2 text-xs text-white/65">{message}</p>}</section></>;
}
