...
export function SpotifyAdminPlayer({ queue }: { queue: SongRequestRecord[] }) {
  ...
  useEffect(() => {
    ...
    void player.connect();
    return () => { player.disconnect(); playerRef.current = null; setDeviceId(""); };
  }, [connected, getAccessToken, sdkReady]);

  useEffect(() => {
    if (!deviceId) return;
    const timer = window.setInterval(() => { playerRef.current?.getCurrentState().then((state) => { if (state) setPlayback(state); }).catch(() => undefined); }, 1000);
