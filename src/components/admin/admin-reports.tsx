    ...
    <>
              ...
              {visible.map((report) => (
                <article key={report.id} className="ios-card p-5 sm:p-6">
                  <div className="flex flex-wrap items-center gap-2">
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-bold ${report.targetType === "post" ? "bg-[#e9f3ff] text-[#007aff]" : "bg-[#f3eafa] text-[#af52de]"}`}
                    >
                  ...
                  {report.detail && (
                    <p className="mt-3 text-sm leading-6">
                      <strong>신고자 설명:</strong> {report.detail}
                    </p>
                  )}
                  {report.status === "pending" ? (
                    <div className="mt-5 flex flex-wrap gap-2 border-t border-[var(--border)] pt-4">
                      ...
                      <button
                        onClick={() => start(report, "edit")}
                        className="inline-flex h-10 items-center gap-1.5 rounded-full bg-[#edf5ff] px-4 text-xs font-bold text-[#007aff]"
                      >
                        <FilePenLine size={14} /> 내용 강제 수정
                      </button>
                      <button
                        onClick={() => start(report, "delete")}
                        className="inline-flex h-10 items-center gap-1.5 rounded-full bg-red-50 px-4 text-xs font-bold text-red-600"
                      >
                        <Trash2 size={14} />{" "}
                        {report.targetType === "post" ? "게시물" : "댓글"} 삭제
                      </button>
                      <button
                        onClick={() => start(report, "dismiss")}
                        className="h-10 rounded-full bg-[#f2f2f7] px-4 text-xs font-bold text-[var(--muted)]"
                      >
                        조치 없이 종결
                      </button>
                    </div>
                    ...
                    <div className="mt-5 rounded-2xl border border-green-100 bg-green-50 p-4 text-sm leading-6 text-green-800">
                      ...
                      <time className="mt-1 block text-xs text-green-700/70">
      ...
      {selected && (
        <div className="fixed inset-0 z-[100] grid place-items-center overflow-y-auto bg-black/30 p-5 backdrop-blur-sm">
          <form
            onSubmit={resolve}
            className="ios-pop my-auto w-full max-w-xl rounded-[28px] bg-white p-6 shadow-2xl sm:p-7"
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                ...
                className="grid size-9 place-items-center rounded-full bg-[#f2f2f7] text-[var(--muted)]"
              >
                <X size={17} />
              </button>
            </div>
            ...
            <button
              disabled={busy}
              className={`mt-5 inline-flex h-12 w-full items-center justify-center rounded-full text-sm font-bold text-white disabled:opacity-50 ${action === "delete" ? "bg-red-600" : "bg-[#007aff]"}`}
            >
              {busy ? (
