...
export function AdminUsers() {
  ...
  useEffect(() => {
    ...
    authenticatedFetch(user, "/api/admin/users").then(async (response) => {
      if (!response.ok) { setMessage(response.status === 403 ? "관리자만 접근할 수 있습니다." : "사용자 목록을 불러오지 못했습니다."); return; }
