      ...
      >
        {links.map(({ href, label, icon: Icon }) => (
          <Link
            key={href}
            href={href}
            className={`flex flex-col items-center gap-0.5 rounded-xl py-1.5 text-[10px] font-medium ${active === href ? "text-[#007aff]" : "text-[var(--muted)]"}`}
          >
            <Icon size={20} strokeWidth={active === href ? 2.5 : 2} />
            {label}
          </Link>
        ))}
      </nav>
