...
export function CommunityBoard() {
  ...
  <div className="mt-7 grid gap-6 lg:grid-cols-[1fr_280px]"><section>{open && user && <form onSubmit={submit} className="ios-card mb-5 grid gap-4 p-5 sm:p-6"><div className="flex items-center gap-2 text-lg font-bold"><span className="grid size-9 place-items-center rounded-[11px] bg-[#007aff] text-white"><PenLine size={18}/></span>새 게시물</div><select name="category" className="rounded-xl border border-[var(--border)] bg-[#f2f2f7] p-3 outline-none focus:border-[#007aff]">{POST_CATEGORIES.map((item) => <option key={item}>{item}</option>)}</select><input name="title" required maxLength={80} placeholder="제목을 입력하세요" className="rounded-xl border border-[var(--border)] bg-[#f2f2f7] p-3 outline-none focus:border-[#007aff]"/><MarkdownEditor name="content" rows={8} placeholder="무슨 이야기를 나누고 싶나요? Markdown 문법을 사용할 수 있어요."/><button disabled={saving} className="justify-self-end rounded-full bg-[#007aff] px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{saving ? "게시 중…" : "게시하기"}</button></form>}{error && <p className="mb-5 rounded-2xl bg-red-50 p-4 text-sm text-red-600">{error}</p>}{state ? <div className="ios-card grid min-h-64 place-items-center p-8 text-center"><div><span className="mx-auto grid size-14 place-items-center rounded-[18px] bg-[#e5f1ff] text-[#007aff]"><MessageCircle size={26}/></span><p className="mt-4 text-[var(--muted)]">{state}</p></div></div> : <div className="space-y-3">{posts.map((post) => <Link key={post.id} href={`/community/${post.id}`} className="ios-card ios-card-interactive stagger-item block p-5"><div className="flex items-center gap-2 text-xs text-[var(--muted)]"><span className="rounded-full bg-[#e5f1ff] px-2.5 py-1 font-medium text-[#007aff]">{post.category}</span><span>{post.authorNickname}</span><span>·</span><time>{formatPostDate(post.createdAt)}</time></div><h2 className="mt-3 text-lg font-bold">{post.title}</h2><p className="mt-2 line-clamp-2 text-sm leading-6 text-[var(--muted)]">{markdownToPlainText(post.content)}</p><div className="mt-4 text-xs text-[var(--muted)]">댓글 {post.commentCount} · 좋아요 {post.likeCount}</div></Link>)}</div>}</section>
