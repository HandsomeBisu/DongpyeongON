...
import { profileFromData, studentProfileSchema, type StudentProfileInput, type UserProfile } from "@/lib/user-profile";
...
type AuthContextValue = {
  ...
  registerWithPassword: (email: string, password: string) => Promise<EmailAuthResult>;
...
function authErrorMessage(caught: unknown, mode: "login" | "register") {
  ...
  if (code === "auth/weak-password") return "비밀번호는 8자 이상으로 안전하게 만들어 주세요.";
...
async function ensureProfile(user: User) {
  ...
  if (!snapshot.exists()) {
    const initial = { uid: user.uid, displayName: user.displayName || "동평 학생", email: user.email, photoURL: user.photoURL, role: "student", onboardingCompleted: false, createdAt: serverTimestamp(), updatedAt: serverTimestamp() };
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(auth, async (next) => {
      try {
        if (next && (!next.email || !isSchoolEmail(next.email))) { await firebaseSignOut(auth); setUser(null); setProfile(null); setError(`@${SCHOOL_EMAIL_DOMAIN} 학교 계정으로만 로그인할 수 있습니다.`); return; }
        if (next && !next.emailVerified) { setUser(null); setProfile(null); return; }
  ...
  const registerWithPassword = useCallback(async (email: string, password: string): Promise<EmailAuthResult> => {
    setError(null);
    const normalized = normalizeEmail(email);
    if (!isSchoolEmail(normalized)) {
      setError(`@${SCHOOL_EMAIL_DOMAIN}로 끝나는 학교 이메일을 입력해 주세요.`);
  ...
  const saveStudentProfile = useCallback(async (input: StudentProfileInput) => {
  ...
  const value = useMemo(() => ({ user, profile, loading, configured: isFirebaseConfigured, error, signIn, signInWithPassword, registerWithPassword, saveStudentProfile, signOut }), [user, profile, loading, error, signIn, signInWithPassword, registerWithPassword, saveStudentProfile, signOut]);
